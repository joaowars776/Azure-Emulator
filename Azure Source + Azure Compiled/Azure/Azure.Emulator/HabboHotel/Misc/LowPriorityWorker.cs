        ...
        internal static void Process(object caller)
        {
            try
            {
                ...
                using (var queryReactor = Azure.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if ((_lastDate == DateTime.Now.ToShortDateString() && clientCount > _lastDatePeak) || _lastDate != DateTime.Now.ToShortDateString())
                        queryReactor.RunFastQuery("REPLACE INTO server_peaks VALUES ('" + DateTime.Now.ToShortDateString() + "', " + clientCount + ")");
