                    ...
                    var client in clients.Where(client => client != null && client.GetHabbo() != null))
                {
                    ...
                    if (ExtraSettings.DIAMONDS_LOOP_ENABLED)
                        if (ExtraSettings.DIAMONDS_VIP_ONLY)
                            if (client.GetHabbo().VIP || client.GetHabbo().Rank >= 6) client.GetHabbo().BelCredits += ExtraSettings.DIAMONDS_TO_GIVE;
                            else client.GetHabbo().BelCredits += ExtraSettings.DIAMONDS_TO_GIVE;
