...
namespace Azure.HabboHotel.Users
{
    ...
    public class Habbo
    {
        ...
        internal Habbo(uint id, string userName, string realName, uint rank, string motto, string look, string gender,
            ...
            Dictionary<int, NaviLogs> naviLogs, bool newNavigator, int dailyCompetitionVotes, uint dutyLevel)
        {
            Id = id;
            UserName = userName;
            RealName = realName;
        ...
        internal void OnDisconnect(string reason)
        {
            ...
            if (!_habboinfoSaved)
            {
                ...
                {
                    queryReactor.SetQuery("UPDATE users SET activity_points = " + ActivityPoints + ", credits = " + Credits + ", seasonal_currency = " + BelCredits + ", online='0', last_online = '" + Azure.GetUnixTimeStamp() + "', builders_items_used = " + BuildersItemsUsed + ", navigator = '" + Azure.BoolToEnum(NewNavigator) + "', navilogs = @navilogs  WHERE id = " + Id + " LIMIT 1;UPDATE users_stats SET achievement_score=" + AchievementPoints + " WHERE id=" + Id + " LIMIT 1;");
