        ...
        internal static UserData GetUserData(string sessionTicket, out uint errorCode)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        dbClient.SetQuery(string.Format("SELECT Count( IsReaded ) FROM xdrcms_minimail WHERE InBin = 0 AND IsReaded = 0 AND SenderId != {0} AND OwnerId = {0}", Convert.ToUInt32(userid)));
                        rowi = dbClient.GetRow();
                    }
                    if (rowi != null)
                        miniMailCount = uint.Parse(rowi[0].ToString());
                }
                catch
                {
                }
