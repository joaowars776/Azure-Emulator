...
namespace Azure.HabboHotel.Items
{
    ...
    internal class MoodlightData
    {
        ...
        internal void UpdatePreset(int preset, string color, int intensity, bool bgOnly, bool hax = false)
        {
            ...
            using (var queryReactor = Azure.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.SetQuery(string.Format(format: "UPDATE items_moodlight SET preset_{0}='{1},{2},{3}' WHERE item_id='{4}'", args: new object[] { text, color, intensity, Azure.BoolToEnum(bgOnly), ItemId }));
        ...
        internal MoodlightPreset GetPreset(int i)
        {
            {
                i--;
                return Presets[i] ?? new MoodlightPreset(colorCode: "#000000", colorIntensity: 255, backgroundOnly: false);
            }
        }
