...
namespace Azure.Messages.Handlers
{
    ...
    partial class GameClientMessageHandler
    {
        ...
        {
            var errorOccured =
                Azure.GetGame().GetModerationTool().UsersHasPendingTicket(Session.GetHabbo().Id);
            if (errorOccured)
            {
                var Ticket = Azure.GetGame().GetModerationTool().GetPendingTicketForUser(Session.GetHabbo().Id);
        ...
        internal void ModGetUserInfo()
        {
            ...
            this.Session.SendNotif(Azure.GetLanguage().GetVar("help_information_error"));
        }
        ...
        internal void ModGetRoomChatlog()
        {
            if (!Session.GetHabbo().HasFuse("fuse_chatlogs"))
            {
                this.Session.SendNotif(Azure.GetLanguage().GetVar("help_information_error_rank_low"));
                return;
            }
            Request.GetInteger();
